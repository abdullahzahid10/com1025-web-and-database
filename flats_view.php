...
$sql = "SELECT FlatID, Flat_DoorNum, Flat_PCode, Flat_ContactNo, Flat_Manager, WifiName, TotalRooms, AdditionalComments from Flat";
$result = $conn-> query($sql);
    
if ($result-> num_rows > 0){
    while($row = $result-> fetch_assoc()){
        echo "<tr><td>". $row["FlatID"] . "</td><td>". $row["Flat_DoorNum"] . "</td><td>". $row["Flat_PCode"] . "</td><td>". $row["Flat_ContactNo"] . "</td><td>". $row["Flat_Manager"] . "</td><td>". $row["WifiName"] . "</td><td>". $row["TotalRooms"] . "</td><td>". $row["AdditionalComments"] . "</td></tr>";
    }
    echo "</table>";
} else {
    echo "0 result";
