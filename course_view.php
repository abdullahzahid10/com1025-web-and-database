...
$sql = "SELECT CourseID, CourseCode, CourseName, CourseLeader from Course";
$result = $conn-> query($sql);
    
if ($result-> num_rows > 0){
    while($row = $result-> fetch_assoc()){
        echo "<tr><td>". $row["CourseID"] . "</td><td>". $row["CourseCode"] . "</td><td>". $row["CourseName"] . "</td><td>". $row["CourseLeader"] ."</td><tr>";
    }
    echo "</table>";
} else {
    echo "0 result";
