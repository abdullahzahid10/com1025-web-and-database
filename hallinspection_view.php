...
<html>
<head>
<title>Hall Inspection - View</title>
<style>
.header {
padding: 30px;
text-align: center;
...
$sql = "SELECT InspectionID, StaffID, InspectionDate, ConditionBefore, ConditionAfter, AdditionalComments from HallInspection";
$result = $conn-> query($sql);
    
if ($result-> num_rows > 0){
    while($row = $result-> fetch_assoc()){
        echo "<tr><td>". $row["InspectionID"] . "</td><td>". $row["StaffID"] . "</td><td>". $row["InspectionDate"] . "</td><td>". $row["ConditionBefore"] . "</td><td>". $row["ConditionAfter"] . "</td><td>". $row["AdditionalComments"] . "</td></tr>";
    }
    echo "</table>";
} else {
    echo "0 result";
